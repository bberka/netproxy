...
public class UdpConnection
{
    ...
    public void Run()
    {
        Task.Run(async () =>
        {
            using (_forwardClient)
            {
                ...
                logger.Info("UDP", "ESTABLISHED",
                    $"{_sourceEndpoint} => {_serverLocalEndpoint} => {_forwardLocalEndpoint} => {_remoteEndpoint}");
                ...
                while (_isRunning)
                    ...
                    catch (Exception ex)
                    {
                        if (_isRunning)
                        {
                            ...
                            logger.Exception(ex, "UDP",
                                "An exception occurred while receiving a server datagram.");
                        }
                    }
            }
        });
    ...
    public void Stop()
    {
        try
        {
            ...
            logger.Warn("UDP",
                $"Closed connection {_sourceEndpoint} => {_serverLocalEndpoint} => {_forwardLocalEndpoint} => {_remoteEndpoint}. {_totalBytesForwarded} bytes forwarded, {_totalBytesResponded} bytes responded.");
            _isRunning = false;
