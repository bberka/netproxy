...
public static class ConnValidator
{
  public static bool Validate(TcpProxy proxy, TcpConnection tcp, out string reason) {
    ...
    if (proxy.Config.ConnectionLimitPerIp != 0) {
      var count = proxy.Connections
                       .Where(x => x.ClientEndPoint.GetIpAddress() == tcp.ClientEndPoint.GetIpAddress()).Count();
    ...
    if (proxy.Config.RequireConnectionToPort != 0) {
      var proxies =
        Statics.TcpProxies.FirstOrDefault(x => x.Config.LocalPort == proxy.Config.RequireConnectionToPort);
      if (proxies is null) {
